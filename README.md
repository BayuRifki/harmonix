...
- **Phase 14.5.1 — Phase 14 deferred-items catch-up (14.1 + 14.2 + 14.3 + 14.4) shipped** — Single-pass close-out of every 14.1 → 14.4 bullet that was marked "deferred to 14.4/14.5" in the first-pass plans. **14.1**: drag-to-reorder sidebar nav via `@dnd-kit/sortable` (grip handle, `PointerSensor` + `KeyboardSensor`, persisted to `uiStore.navOrder`, new "Reset order" button); `FocusTrap` integrated into `Modal` and `CommandPalette` (Tab cycle + focus restoration on close); full focus-visible audit across Sidebar, MiniPlayer, Command Palette, drag handles, playlist cards. **14.2**: two new visualizer variants — `ParticleField` (36-80 floating particles driven by bass + treble FFT bins) and `StereoOscilloscope` (512-bin time-domain split into two L/R strokes); `useEffectiveVisualizerQuality()` auto-degrades to 20 FPS on `navigator.hardwareConcurrency < 4`; new `ThemePanel` adds manual theme override (auto/brand/custom hex with live `<input type="color">` + text validation), glass intensity (off/subtle/strong drives `data-glass` on `<html>`), per-visualizer Settings toggles + Performance mode preset; `[data-adaptive]` opt-in CSS hook. **14.3**: `LyricsPanel` fetches from LRClib with 5-min localStorage cache, `AbortController` per track change, `findActiveLineIndex` binary search for karaoke highlighting, click-to-seek; `useGestures` hook detects swipe (4 directions) / pinch / double-tap and dispatches next/previous/volume/play-pause; MiniPlayer artwork is now a dnd-kit drop target (track → queue); MiniPlayer snap-to-edge on `mouseup`. **14.4**: full dnd-kit integration via `GlobalDndProvider` (track → playlist, queue reorder via new `playerStore.moveQueueItem/insertIntoQueue/removeFromQueue`, file drop → library scan via Electron `File.path`, sidebar nav reorder, mini-player queue insert); `ImageLoader` with LQIP blur-up + error fallback; `Suspense`-based route loading with `RouteFallback` skeleton variants; `HorizontalScroller` with snap points + scroll indicators; `showExitAnimations` toggle on `uiStore`. New files: `src/lib/dndData.ts`, `src/lib/lyrics.ts`, `src/hooks/useGestures.ts`, `src/hooks/useVisualizerQuality.ts`, `src/components/dnd/{GlobalDndProvider,DraggableTrack,PlaylistDropZone,FileDropZone}.tsx`, `src/components/sidebar/SortableNavItem.tsx`, `src/components/ui/{ImageLoader,HorizontalScroller}.tsx`, `src/components/a11y/RouteFallback.tsx`, `src/features/lyrics/LyricsPanel.tsx`, `src/features/settings/ThemePanel.tsx`. `src/components/ui/Modal.tsx` + `src/components/command/CommandPalette.tsx` now wrap content in `FocusTrap`. `src/components/layout/Sidebar.tsx` rewritten as a `DndContext` + `SortableContext`. `src/stores/uiStore.ts` extended with 11 new persisted fields (`navOrder`, `sidebarLayout`, `visualizerQuality`, `animationIntensity`, `glassIntensity`, `themeAccentMode`, `customAccentHex`, `enabledVisualizers`, `showExitAnimations`, `showSnapPoints`, `showScrollIndicators`). `src/stores/playerStore.ts` gained `moveQueueItem` / `insertIntoQueue` / `removeFromQueue`. `src/features/nowPlaying/NowPlayingView.tsx` mounts the lyrics panel, gesture hook, and two new visualizer modes. `src/features/miniPlayer/MiniPlayerView.tsx` is a dnd-kit drop zone and snaps to screen edges. `src/App.tsx` wraps the entire tree in `GlobalDndProvider` + `<Suspense>`. 12 new test files (62 new tests): lyrics (11), visualizer variants (5), visualizer quality (1), uiStore extended (15), playerStoreQueue (11), dndData (6), imageLoader (3), lyricsPanel (3), horizontalScroller (2), globalDndProvider (2), playlistDropZone (2), routeFallback (3). **689/689 tests pass** across 74 test files (was 627/62 — +62 tests, +12 files). Lint clean, typecheck clean, build clean. See `docs/PLANNING.md` §14.5.1 for the full scope.
