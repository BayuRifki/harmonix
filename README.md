...
- ✅ **Accessibility audit (Phase 14.7 Batch 2)** — `prefers-reduced-motion` everywhere (vinyl, particles, transitions, magnetic hover, ripples, stagger), `prefers-color-scheme: light` parity, `aria-live` via `PlayerAnnouncer`, WCAG AA contrast, logical tab order, `prefers-contrast: more` high-contrast, screen reader tested
- ✅ **Light / Dark / System theme** — Theme selector in Settings (Phase 14.7 Batch 2)
- ✅ **Internationalization (i18n) ready** — `src/i18n/en.ts` catalog, `Intl.DateTimeFormat`/`Intl.NumberFormat`, locale-aware formatting, RTL-ready logical CSS (Phase 14.7 Batch 2)
- ✅ **Performance settings panel** — visualizer quality (Auto/High/Off), animation intensity (Full/Reduced/Off), React.memo on heavy lists, `useDeferredValue` search (Phase 14.4 + 14.7)
- ✅ **Player settings panel** — crossfade preview, trackpad gestures toggle, mini-player defaults (size + always-on-top) (Phase 14.7 Batch 2)
- ✅ **Keyboard settings panel** — fully editable shortcuts with inline key capture, per-shortcut enable/disable, reset to defaults (Phase 14.7 Batch 2)
- ✅ **Local telemetry (opt-in)** — render/interaction/latency/error tracking, localStorage persistence, diagnostics view (Phase 14.7 Batch 2)

### Engineering / DX

- ✅ Light / Dark / System theme (Phase 7 + 14.7)
...
- ✅ **792 tests across 82 files** (Phase 14.7 Batch 1 + 2 — MiniPlayer Lucide icons + snap-to-edge IPC, HeroPlayer Add to Favorites, focus-ring audit, accessibility audit complete (prefers-reduced-motion, light mode parity, aria-live, WCAG AA, tab order, prefers-contrast), i18n prep (i18n/en.ts, Intl.DateTimeFormat, locale-aware numbers, RTL readiness), React.memo on heavy lists, useDeferredValue search, PlayerPanel (crossfade/gestures/mini-player), KeyboardShortcutsPanel (editable shortcuts, inline key capture, reset defaults), telemetryStore (opt-in local telemetry), Dark/Light/System theme, ifferences & 15 pre-existing warnings)
- ✅ **1002 / 1029 tests across 111 files** (For You polish + desktop installer — `+29` in `scoring.test.ts` for the 4th "personal" signal + audio-similarity re-ranking, `+20` in new `spotifyAudioFeatures.test.ts` for the URL builder + response parser). 27 failures are all pre-existing `better-sqlite3` env issues (`NODE_MODULE_VERSION 137` vs the test runner's `115`) in `playlistRepository` / `trackRepository` — not from this work. `npm run dist:win` now produces a real `Harmonix-0.1.0-x64-Setup.exe` NSIS installer.
...
- 🔜 Code signing + first public release (v0.1.0) — NSIS installer pipeline is in place (`npm run dist:win` ships a working installer); only the cert + signing hooks are left.
...
| M19       | For You algorithm polish + desktop installer                            | ✅ Done                      |

## Recent Progress (active session)

- **Milestone 19 — For You polish + desktop installer shipped**:
  - **Spotify audio-features API** — new `getAudioFeatures(trackIds)` on the Spotify client, exposed as `window.api.auth.spotifyAudioFeatures(trackIds)` (IPC chunks the input into Spotify's 100-id-per-request batches). Returns danceability / energy / valence / tempo / acousticness / instrumentalness / speechiness / liveness. Skips gracefully on any failure.
  - **"Personal" signal in the recommender** — the user's top-played tracks from the local listening-history store are synthesised into `Track` objects and injected as a 4th signal in `mergeRecommendations` (default weight 0.15). Helps the "For You" rail surface trusted tracks even when the content / session / history search signals miss them.
  - **Audio-similarity re-ranking** — `cosineSimilarity(a, b)` + `rerankByAudioSimilarity(tracks, featuresMap, currentFeatures, weight)` in `scoring.ts`. `useHybridRecommendations` calls `maybeRerankByAudioFeatures` after the merge to boost Spotify recommendations whose danceability / energy / valence / tempo are close to the current track. No-ops cleanly when not on Spotify or not authenticated. Default weight 0.3.
  - **Stronger artist-diversity algorithm** — replaced head+tail with a two-phase greedy slot-by-slot interleaving (Phase 1 fills uncapped, Phase 2 round-robins overflow by artist). The 6-artist / 1-per-artist integration test now deterministically achieves 1-Coldplay-in-top-6.
  - **NSIS installer for Windows** — `npm run dist:win` now produces a real `Harmonix-0.1.0-x64-Setup.exe` (~110 MB) that bundles the Electron app + `yt-dlp.exe` and installs via the standard NSIS wizard (start menu + desktop shortcuts, configurable install dir, uninstaller). The release CI workflow (`.github/workflows/release.yml`) already builds installers for all three platforms in parallel on tagged releases. **Test count**: 1002/1029 tests pass across 111 files (+29 in `scoring.test.ts`, +20 in new `spotifyAudioFeatures.test.ts`); 27 pre-existing better-sqlite3 env failures, 0 new regressions.
  - **Cleanup** — removed stray `test_output.txt` (281 KB) and unreferenced `resources/brand-guide.png` (byte-identical duplicate of `logo.png`); fixed `.gitignore` to cover both `test_output.txt` and `test-output.txt`.
...
**Next: Phase 14.7 Batch 2 (Planned)** — Accessibility, Settings & Performance:

- **Accessibility Audit** — reduced-motion everywhere, light mode parity, aria-live, contrast audit, tab order, high-contrast, screen reader tests
- **i18n Prep** — extract strings to `src/i18n/en.ts`, Intl.DateTimeFormat, locale-aware numbers, RTL readiness
- **Performance** — React.memo on heavy lists, extend virtualization (QueueDrawer, PlaylistDetailView), useDeferredValue for search, bundle analysis
- **Settings** — Appearance (theme/accent/glass), Player (mini-player/crossfade/gestures), Keyboard (editable shortcuts/reset)
- **Telemetry** — local-only opt-in (render times, latencies, errors), SQLite storage, diagnostics panel
