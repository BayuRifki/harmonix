...
- **Phase 14.1 completion — Keyboard Navigation, Command Preview, Search Filters, FocusTrap** — Closed all remaining Phase 14.1 + 14.4 + 14.5 deferred items. (1) **Full keyboard navigation system**: new `src/hooks/useKeyboardNavigation.ts` (9 tests) dispatches 20 categorized shortcuts (Play/Pause, Seek 5s, Prev/Next, Volume, Mute, Shuffle, Repeat, Queue, List j/k/g/G/h/L, Command Palette, Mini-player, Help Overlay) with per-shortcut enable/disable in a new persisted `src/stores/keyboardSettingsStore.ts` (6 tests, `harmonix.keyboard` localStorage key via Zustand `persist` middleware). Mounted in `App.tsx` alongside the existing `useKeyboardShortcuts` hook. (2) **Keyboard help overlay** (`src/components/keyboard/KeyboardHelpOverlay.tsx`, 7 tests): searchable modal opened by `?` with per-shortcut toggle switches and a Reset button. (3) **Inline command-palette preview pane** (`src/components/command/CommandPreview.tsx`, 7 tests): right-side metadata preview with artwork, title, artist, source/duration/album and a Play action for tracks. Width bumped `max-w-xl` → `max-w-3xl` in CommandPalette. (4) **Search filters** (`src/features/search/SearchFiltersBar.tsx` + `src/features/search/searchFilters.ts`, 11 tests): source / duration / artist filter chips with URL deep-linking (`?src=spotify,local&dur=short&artist=radiohead`); `SearchView` filters results and rewrites the URL on change. (5) **FocusTrap** utility (`src/components/ui/FocusTrap.tsx`, 4 tests): reusable `Tab`/`Shift+Tab` focus cycling + focus restoration on unmount. (6) **KeyboardShortcutsPanel** (`src/features/settings/KeyboardShortcutsPanel.tsx`): now shows the full categorized shortcut list and links to the help overlay. **627/627 tests pass** (was 578, +49). Lint clean, typecheck clean. No git commit. Phase 14.1 is now fully shipped.
