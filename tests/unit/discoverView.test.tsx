...
describe('DiscoverView', () => {
  ...
  it('shows the first-time hint when history and session are empty', () => {
    render(wrap(<DiscoverView />));
    expect(screen.getByTestId('discover-hint')).toBeInTheDocument();
    expect(screen.getByText(/Play 2-3 tracks/i)).toBeInTheDocument();
  });

  it('hides the first-time hint when history is non-empty', () => {
  ...
  it('renders 3 row sections: Top picks, From history, By mood', () => {
    render(wrap(<DiscoverView />));
  ...
  it('Top picks section has a different subtitle when no current track', () => {
    render(wrap(<DiscoverView />));
  ...
  it('Top picks section subtitle mentions current track when one is playing', () => {
  ...
  it('renders mood section chips only when the sample matches the section mood', () => {
  ...
  it('From history section shows history entries when present', () => {
    useListeningHistoryStore.getState().add(t);
    render(wrap(<DiscoverView />));
    expect(screen.getByText(/of 1 recent plays/i)).toBeInTheDocument();
  });

  it('"From history" shows empty state when history is empty', () => {
    render(wrap(<DiscoverView />));
